...
if (
  aggregatedDeliveryReceipt.failed !== 'none' ||
  aggregatedDeliveryReceipt.undelivered !== 'none'
) {
  // handle error
}
...
detailedDeliveryReceipts.map((detailedDeliveryReceipt) => {
  ...
  if (
    !detailedDeliveryReceipt.status === 'undelivered' &&
    !detailedDeliveryReceipt.status === 'failed'
  ) {
    return;
  }
